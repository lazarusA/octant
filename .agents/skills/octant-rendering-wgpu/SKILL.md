...
- **Overlays Architecture Note**: Direct WGPU offscreen rendering bypasses `egui` UI layers. If canvas UI elements (vector colorbars, axes, coordinate ticks) must be present in the output, render through the full viewport canvas pipeline with `ViewportCommand::Screenshot` cropped to `canvas_rect`.
