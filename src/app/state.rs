...
impl StoreKind {
    ...
    /// Automatically infers the most appropriate `StoreKind` from a given target URI, file path, or directory.
    pub fn infer_from_target(target: &str) -> Self {
        let trimmed = target.trim();
        if trimmed == "procedural://volume4d" {
            return StoreKind::ProceduralVolume4D;
        }
        if trimmed == "procedural://matrix"
            || trimmed == "procedural://random"
            || trimmed == "procedural://matrix2d"
        {
            return StoreKind::ProceduralRandom;
        }
            ...
            || trimmed.starts_with("az://")
        {
            if trimmed.to_lowercase().contains("icechunk") {
                return StoreKind::RemoteIcechunk;
            } else {
                return StoreKind::RemoteZarr;
            }
        }
        if trimmed.starts_with("netcdf://") || trimmed.starts_with("hdf5://") {
            return StoreKind::LocalNetCdf;
        }
        ...
        if path.exists() {
            if path.is_file() {
                ...
                if matches!(ext.as_str(), "zarr" | "zip") {
                    return StoreKind::LocalZarr;
                }

                // Check magic bytes if extension is unusual
...
mod tests {
    ...
    #[test]
    fn test_infer_from_target_procedural() {
        assert_eq!(
            StoreKind::infer_from_target("procedural://volume4d"),
            StoreKind::ProceduralVolume4D
        );
        assert_eq!(
            StoreKind::infer_from_target("procedural://matrix"),
            StoreKind::ProceduralRandom
        );
        assert_eq!(
            StoreKind::infer_from_target("procedural://random"),
            StoreKind::ProceduralRandom
        );
        assert_eq!(
            StoreKind::infer_from_target("procedural://matrix2d"),
            StoreKind::ProceduralRandom
        );
    }

    #[test]
    fn test_infer_from_target_remote() {
        assert_eq!(
            StoreKind::infer_from_target("https://s3.bgc-jena.mpg.de/esdl.zarr"),
            StoreKind::RemoteZarr
        );
        assert_eq!(
            StoreKind::infer_from_target("s3://bucket/dataset.zarr"),
            StoreKind::RemoteZarr
        );
        assert_eq!(
            StoreKind::infer_from_target("https://example.com/repo_icechunk"),
            StoreKind::RemoteIcechunk
        );
        assert_eq!(
            StoreKind::infer_from_target("s3://bucket/icechunk_store"),
            StoreKind::RemoteIcechunk
        );
    }

    #[test]
    fn test_infer_from_target_netcdf_and_hdf5_extensions() {
        assert_eq!(
            StoreKind::infer_from_target("/data/sample.nc"),
            StoreKind::LocalNetCdf
        );
        assert_eq!(
            StoreKind::infer_from_target("/data/sample.nc4"),
            StoreKind::LocalNetCdf
        );
        assert_eq!(
            StoreKind::infer_from_target("/data/sample.h5"),
            StoreKind::LocalNetCdf
        );
        assert_eq!(
            StoreKind::infer_from_target("/data/sample.hdf5"),
            StoreKind::LocalNetCdf
        );
        assert_eq!(
            StoreKind::infer_from_target("/data/sample.cdf"),
            StoreKind::LocalNetCdf
        );
        assert_eq!(
            StoreKind::infer_from_target("netcdf:///path/to/data"),
            StoreKind::LocalNetCdf
        );
    }

    #[test]
    fn test_infer_from_target_directories_and_zarr() {
        assert_eq!(
            StoreKind::infer_from_target("/path/to/dataset.zarr"),
            StoreKind::LocalZarr
        );
        assert_eq!(
            StoreKind::infer_from_target("/path/to/icechunk_repo"),
            StoreKind::LocalIcechunk
        );
    }
}
