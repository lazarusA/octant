...
/// and preserving existing extract_store_variables as an ultimate fallback.
pub fn extract_store_variables_consolidated(
    ...
    if variables.is_empty() {
        return extract_store_variables(store, base_url);
    }
...
fn variable_info_from_array<TStorage: ?Sized + ReadableStorageTraits>(
